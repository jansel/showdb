...
from config import showdb_file, download_dir, filters_file, titlere_SE, \
  titlere_DATE
...
normalizeShowName = (lambda s: normalizeShownameRe.sub('', s)
                                                  .replace('.', ' ')
                                                  .replace('_', ' ').strip())
...
def makeDateStr(d):
  ...
  for f in ["%b %d %Y", "%b %d, %Y", "%Y-%m-%d", "%Y.%m.%d", "%m/%d/%Y",
            "%m/%d/%y"]:
    try:
...
def splitShowEp(title):
  for reg in titlere_SE:
    ...
    if m is not None:
      return (
      normalizeShowName(m.group(1)), makeSeEpStr(m.group(2), m.group(3)))
...
def mkshowfilter():
  filters = map(lambda x: re.compile(x + '$', re.IGNORECASE),
                loadlist(filters_file))
...
def add(name, episode, src, url, method='DB'):
  ...
  try:
    rc = db.execute(
      "INSERT INTO shows(name,episode,src,url,created) VALUES (?,?,?,?,?)",
...
def downloadShow(name, episode, t=int(time.time())):
  db = opendb()
  r = db.execute(
    "SELECT count(*) FROM shows WHERE name=? AND episode=? AND downloaded>0",
    (name, episode))
...
def markChecked(name, episode, t):
  db = opendb()
  db.execute(
    "UPDATE shows SET checked=? WHERE checked=0 AND created<=? AND name=? and episode=?",
    (t, t, name, episode))
  db.commit()
  db.close()
...
def checkNew():
  ...
  db = opendb()
  rows = db.execute(
    "SELECT DISTINCT name, episode FROM shows WHERE checked=0").fetchall()
  db.close()
