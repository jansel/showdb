...
def parseSummaryBased(show, feed_url):
  ...
  if showinfo.has_key('show name'):
    ...
    return showdb.add(showinfo['show name'], ep, show['title']+" "+feed_url, show['link'], method='RSS')
...
def parseTitleBased(show, feed_url):
  ...
  return showdb.add(name, ep, show['title']+" "+feed_url, show['link'], method='RSS')
...
def parseFeed(feed_url, feed):
  ...
  for e in feed['entries']:
    ...
    if rv is None:
      errors+=1
  print "RSS: fetched ", feed_url," %d existing %d new %d errors"%(existing,found,errors)
